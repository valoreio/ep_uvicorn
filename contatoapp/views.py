...
def contato(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                # O reg.save() está na funcao da linha abaixo que encontra-se no arquivo contatoapp/tasks.py
                save_postview_taskscelery(request, reg)

#                try:
#                    #coloca o email para o comercial na fila de tarefas assincronas
#                    # verificar porque quando entra nesta funcao ocorre: 504 Gateway Time-out" ao renderizar o submit no form html
...
#                           messages.success(request, "Register inserted and emails were sent to the queue. We will advice you accordingly! Take a look at your spam email folder")
...
#                    messages.error(request, "The request wasn't sent to the queue because of error while loading the task queue-e2: {0}".format(e))
                ...
                messages.error(request, "The request was not inserted into database: ".format(err))
